...
% 		Line number: 86
%  			Loop variable 'i' changed to 'j', as i is not totally 
%  		defined. The change was made to fix an error that occurred 
%  		occasionally during GMM modeling iwth a full covariance matrix.  
%  		There are still occasional problems with full covarience GMM 
%  		modeling at line 85.
...
case 'spherical'
  ...
  case 'full'
	...
	mix.covars(:,:,j) = mix.covars(:,:,j) + GMM_WIDTH.*eye(mix.nin);  %CHANGE 1 (XL)
      end
    end
  case 'ppca'
    for j = 1:mix.ncentres
